...
# Load workbooks.
pA2_wb  = xl.load_workbook("Data/LModeProbes.xlsx", data_only=True)
pA32_wb = xl.load_workbook("Data/A32.xlsx", data_only=True)
pA20_wb = xl.load_workbook("Data/A20.xlsx", data_only=True)
...
pA2U_loc_cells = pA2_sheet["E2:E20"]
pA20D_loc_cells = pA20_sheet["A2:A21"]
pA32D_loc_cells = pA32_sheet["A2:A21"]
...
pA2U_loc       = cell_to_value(pA2U_loc_cells)
pA20D_loc      = cell_to_value(pA20D_loc_cells)
pA32D_loc      = cell_to_value(pA32D_loc_cells)
...
# Create plot with each areal profile on it. Seems like 34 font size is good.
plt.rcParams.update({'font.size': 34})
plt.errorbar(x=pA2U_rminrsep[:-1], y=pA2U_areal[:-1], yerr=pA2U_arealerr[:-1], capsize=3, fmt="g", label="A2-OTF")
plt.errorbar(x=pA32D_rminrsep[:-1], y=pA32D_areal[:-1], yerr=pA32D_arealerr[:-1], capsize=3, fmt="y", label="A32-OTF")
plt.errorbar(x=pA20D_rminrsep[:-1], y=pA20D_areal[:-1], yerr=pA20D_arealerr[:-1], capsize=3, fmt="r", label="A20-OTF")
#plt.errorbar(x=pA2U_loc[:-1], y=pA2U_areal[:-1], yerr=pA2U_arealerr[:-1], capsize=3, fmt="g", label="AU2")
#plt.errorbar(x=pA32D_loc[:-1], y=pA32D_areal[:-1], yerr=pA32D_arealerr[:-1], capsize=3, fmt="y", label="AD32")
#plt.errorbar(x=pA20D_loc[:-1], y=pA20D_areal[:-1], yerr=pA20D_arealerr[:-1], capsize=3, fmt="r", label="AD20")
plt.xlabel("Location (cm)")
plt.ylabel(r"$\mathrm{W\ Areal\ Density\ (10^{15}\ atoms/cm^{2})}$")
#plt.ylim([0, 0.1])
...
plt.errorbar(x=pA2U_loc[:-1], y=pA2U_areal[:-1], yerr=pA2U_arealerr[:-1], capsize=3, label="A2-OTF")
plt.errorbar(x=pA2D_loc[:-1], y=pA2D_areal[:-1], yerr=pA2D_arealerr[:-1], capsize=3, label="A2-ITF")
