...
# tcorrscan = [3e-6, 5e-6, 10e-6, 0.5e-6, 0.1e-6]
tcorrscan = [5e-6, 10e-6, 0.5e-6, 0.1e-6]  # Made the first one base. Simpler to just say 3e-6 instead of 1.5e-6.
problabels = [5, 6, 7, 8]
ops_epol = []
ops_prob = []
ops_tcorr = []
...
for i in range(2, len(tcorrscan)+2):
    print("{}/{}".format(i, len(tcorrscan)))
    case3 = "d3d-167196-fluc-002-tcorrscan{}.nc".format(i)
    ops_tcorr.append(oedge_plots.OedgePlots(root + case3))
# op_base = oedge_plots.OedgePlots(root + "d3d-167196-fluc-002-test.nc")
# op_base = oedge_plots.OedgePlots(root + "d3d-167196-fluc-002.nc")
op_base = oedge_plots.OedgePlots(root + "d3d-167196-fluc-002-tcorrscan1.nc")
...
def get_rad_w_prof(op):
    div_data = op.along_line(2.17, 2.36, -0.188, -0.188, "DDLIMS", charge="all", scaling=op.absfac, xtype="psin")
    div_psin = np.array(div_data["psin"])
    nonan = ~np.isnan(div_psin)
    div_psin = div_psin[nonan]
    div_absfac = op.absfac
    div_nw = np.array(div_data["DDLIMS"])[nonan]
...
for i in range(0, len(probscan)):
    ...
    nws_prob.append(nw)
for i in range(0, len(tcorrscan)):
    psin, nw = get_rad_w_prof(ops_tcorr[i])
    psins_tcorr.append(psin)
    nws_tcorr.append(nw)
...
psin_labels_epol = [1.017, 1.125, 1.19, 1.19]
...
for i in range(0, len(epolscan)):
    ...
    # Pretty much overlaps with 2,000, so don't include it
    if epolscan[i] == 1500:
        continue

    ax1.plot(psins_epol[i], nws_epol[i], lw=2, linestyle="-", color="tab:pink")
    if epolscan[i] == 2000:
        ax1.text(1.22, 2e13, "2,000 V/m", fontsize=7, bbox={"facecolor": "w", "edgecolor": "k"},
                horizontalalignment="center")
    else:
        label_at_psin(ax1, psins_epol[i], nws_epol[i], epolscan[i], psin_labels_epol[i])
for i in range(0, len(probscan)):
...
for i in range(0, len(tcorrscan)):
    ax3.plot(psins_tcorr[i], nws_tcorr[i], lw=2, linestyle="-", color="tab:pink")
    # label_at_psin(ax3, psins_tcorr[i], nws_tcorr[i], tcorrscan[i], psin_labels[i])
    if i == 0:
        ax3.text(1.25, 1.5e13, tcorrscan[i], fontsize=7, bbox={"facecolor": "w", "edgecolor": "k"},
                 horizontalalignment="center")
    elif i == 1:
        ax3.text(1.25, 5e13, tcorrscan[i], fontsize=7, bbox={"facecolor": "w", "edgecolor": "k"},
                 horizontalalignment="center")
    elif i == 2:
        ax3.text(1.25, 3e11, tcorrscan[i], fontsize=7, bbox={"facecolor": "w", "edgecolor": "k"},
                 horizontalalignment="center")
    elif i == 3:
        ax3.text(1.05, 5e10, "{:} s".format(tcorrscan[i]), fontsize=7, bbox={"facecolor": "w", "edgecolor": "k"},
                 horizontalalignment="center")
ax3.text(1.25, 4e12, 3e-6, fontsize=7, bbox={"facecolor": "w", "edgecolor": "k"},
                 horizontalalignment="center")
...
ax3.plot(psin_base, nw_base, color="k", lw=3)
ax3.plot(psin_base, nw_base, color="tab:pink", lw=2)
label_at_psin(ax2, psin_base, nw_base, 0.5, 1.15)
# label_at_psin(ax1, psin_base, nw_base, 1000, 1.17)
ax1.text(1.19, 4e12, "1,000", fontsize=7, bbox={"facecolor": "w", "edgecolor": "k"},
                horizontalalignment="center")
...
ax3.text(0.81, 4e15, "c)", fontsize=10)
ax1.set_title(r"$\mathdefault{E_\theta}$ Scan")
ax2.set_title("p+ Scan")
ax3.set_title(r"$\mathdefault{\tau_{corr}}$ Scan")
