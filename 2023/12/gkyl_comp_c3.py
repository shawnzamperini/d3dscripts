...
class Plasma3:
    ...
    def __init__(self, root, fname, fstart, fend, skip_ti=False, skip_phi=False, skip_upar=False, car_flag=False):
        """
        ...
        for t in range(fstart, fend, step):
            ...
            if car_flag:
                ...
                # carbon temperature
                d = pg.GData("%s_carbon_Temp_%d.bp" % (fp, t))
                dg = pg.GInterpModal(d, 1, 'ms')
...
#plasma_c3 = Plasma3("/Users/zamperini/Documents/d3d_work/gkyl_files/d3d-167196-v4-c3/", "d3d-167196-v4-c3", 600, 999,
#                    skip_ti=False, skip_phi=False, skip_upar=True, car_flag=True)  # 0-999

# Gkyell run with only e and D, matched to RCP data pretty well.
plasma = Plasma3("/Users/zamperini/Documents/d3d_work/gkyl_files/d3d-190412-v2/", "d3d-190412-v2", 700, 999,
                 skip_ti=False, skip_phi=False, skip_upar=True)  # 0-999

# Gkyell run where I have added a 2% C3+ background to the above run.
plasma_c3 = Plasma3("/Users/zamperini/Documents/d3d_work/gkyl_files/d3d-190412-v2/", "d3d-190412-v2", 700, 999,
                    skip_ti=False, skip_phi=False, skip_upar=True, car_flag=True)  # 0-999
...
# Plot of the carbon concentration at a time slice.
tslice = 200
ne_c3 = plasma_c3.ne[tslice, :, :, midz]
ni_c3 = plasma_c3.ni[tslice, :, :, midz]
nc_c3 = plasma_c3.nc[tslice, :, :, midz]
...
cont = ax.contourf(plasma.r, plasma.p, nc_c3.T / ni_c3.T, zorder=10, levels=np.linspace(0.00, 0.06, 11), cmap="inferno")
cbar = fig.colorbar(cont, cax=cax, extend="both")
cbar.set_label(r"$\mathdefault{n_C\ /\ n_D}$", fontsize=fontsize)
