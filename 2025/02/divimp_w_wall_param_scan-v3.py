...
xvalues = np.array(list(zip(dperps, ne_charges, ne_fluxes)))
#xvalues = xvalues[:5]

# Some quick 2D plots that are used in the presentation
ncpath = "/home/zamp/oedge_files/d3d-w-wall-param-scan-v3-all-1.nc"
op = oedge_plots.OedgePlots(ncpath)
...
for sput_region in sput_regions:
	...
	#for i in range(0, 5):
		...
		# Load case
		ncpath = ("/home/zamp/oedge_files/d3d-w-wall-param-scan-v3-{}-{}.nc"
...
def model(x, a, b, c, d):
	...
	# Concentration is by definition 1.0 exponent here, so effectively remove
...
for i in range(len(sput_regions)):
	...
	key = sput_regions[i]
	axs[i].scatter(fit_dict[key], fom_vals_dict[key], s=75, 
	...
	text = lhs + r"$\mathdefault{\sim D_r^{" + "{:.1f}".format(popt[0]) \
		+ r"}\ Z_{Ne}^{" + "{:.1f}".format(popt[1]) \
		+ r"}\ f_{Ne}^{" + "{:.1f}".format(popt[2]) + r"}}$"
