...
# Let's assume the divertors have a high prompt redeposition fraction of about 99%,
# and everywhere else has a 50% chance.
#fprompt = {"all":0.0, "sh":0.99, "id":0.99, "ub":0.50, "iw":0.50, "ow":0.50, "lb":0.50}
fprompt = {"all":0.0, "sh":0.00, "id":0.00, "ub":0.00, "iw":0.00, "ow":0.00, "lb":0.00}
...
for sput_region in sput_regions:
    ...
    for i in range(0, xvalues.shape[0]):
        for imp in ["w", "fe"]:
            ...
            # W only, assume Fe does not promptly redeposit appreciably.
            if imp == "w":
                nz_s *= (1.0 - fprompt[sput_region])
...
#              ["sh", "id", "ub", "ow", "lb", "iw"]
no_plot_bars = [True, True, False, False, False, False]
