...
xvalues = np.array(list(zip(dperps, ne_fluxes, wall_tes, machs)))
#xvalues = xvalues[:3]
...
core_R = 1.7
core_vol = (core_rad**2 * np.pi) * (2 * np.pi * core_R)
...
# Variables to each cases W, Fe concentration for the plot
c_w = []
c_fe = []
...
for sput_region in sput_regions:
    ...
    for i in range(0, xvalues.shape[0]):
        ...
        for imp in ["w", "fe-comp"]:
            ...
            # Load case
            ncpath = ("{}/d3d-w-wall-param-scan-v6-{}-{}-{}.nc"
            ...
            # Assign to each array that is used for this case's plot
            if sput_region == "all":
                if imp == "w":
                    c_w.append(cz_s)
                else:
...
# Setup a plot that will have the concentrations along the first ring in the core
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8, 5), sharey=True)

# Plot concentrations on plot
for i in range(len(c_w)):
    ax1.plot(s, c_w[i], alpha=0.5, color="k", lw=3)
    ax1.plot(s, c_w[i], alpha=0.5, color="tab:red", lw=2)
    ax2.plot(s, c_fe[i], alpha=0.5, color="k", lw=3)
    ax2.plot(s, c_fe[i], alpha=0.5, color="tab:red", lw=2)
...
ax1.set_title("W Concentration", fontsize=fontsize)
ax2.set_title("Fe Concentration", fontsize=fontsize)
ax1.set_yscale("log")
...
no_plot_bars = [False, False, False, False, False, False, False]
...
# half the caps from being cut off since they straddle the edge of the plots
cap_width = 0.14
buf = 0.02
...
ax1.barh(y, avg_prad_wfe, 0.75, tick_label=bar_regions_words, edgecolor="k", color="lightskyblue")
ne_str = r"$\mathdefault{10^{20}\ m^{-3}}$"
xlabel = "Prad @ {} keV ".format(int(core_te/1e3)) + ne_str
ax1.set_xlabel("W/Fe Ratio: " + xlabel, fontsize=fontsize)
ax1.tick_params(axis="both", which="major", labelsize=fontsize-2)
fig.tight_layout()
plt.show()
