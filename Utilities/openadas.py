...
class OpenADAS:
    ...
    def coronal_equil_frac_abundance(self, atom_num, iz_rate_df, rec_rate_df, te, ne):
        """

        """

        # Fractional abundances, will normalize afterwards
        frac_abund = np.zeros(atom_num + 1)

        # Neutrals start at 1 so we can calculate the rest
        frac_abund[0] = 1

        # Loop through one charge at a time
        for charge in range(atom_num):

            # Load rate coefficients
            iz_rate = self.get_rate_coef(iz_rate_df, te, ne, charge=charge+1)
            rec_rate = self.get_rate_coef(rec_rate_df, te, ne, charge=charge+1)

            # Abundance is the previous abundance * ionization divided by recombination
            frac_abund[charge + 1] = frac_abund[charge] * iz_rate / rec_rate 

        # Normalize
        frac_abund /= frac_abund.sum()
        return frac_abund
